...
public struct SP4ALeg: Codable, Equatable, Sendable {
...
public enum SP4AValidationError: String, Error, Equatable {
    case duplicateLeg, missingLeg, invalidRule, invalidVerdict, invalidProvenance, invalidAggregate, invalidRunnerSourceSet
}

public struct SP4AEvidence: Codable, Equatable, Sendable {
    public static let requiredLegIDs = Set(Phase0Registry.legRules.keys.filter { $0.hasPrefix("sp4a.") })
    public var schemaVersion: Int
    public var spikeID: String
    public var legs: [SP4ALeg]
    public var verdict: Verdict
    public var runnerSourceSha256: [String: String]

    public init(schemaVersion: Int = 1, legs: [SP4ALeg], verdict: Verdict, runnerSourceSha256: [String: String]) {
        self.schemaVersion = schemaVersion; spikeID = "SP-4A"; self.legs = legs; self.verdict = verdict
        self.runnerSourceSha256 = runnerSourceSha256
    }

    public func validate() throws {
        let grouped = Dictionary(grouping: legs, by: \.legID)
        if grouped.values.contains(where: { $0.count != 1 }) { throw SP4AValidationError.duplicateLeg }
        guard Set(grouped.keys) == Self.requiredLegIDs else { throw SP4AValidationError.missingLeg }
        guard schemaVersion == 1, spikeID == "SP-4A" else { throw SP4AValidationError.invalidProvenance }
        guard Set(runnerSourceSha256.keys) == SP4ARunnerBinding.sourcePaths,
              runnerSourceSha256.values.allSatisfy({ $0.isLowercaseSHA256 && $0 != String(repeating: "0", count: 64) }) else {
            throw SP4AValidationError.invalidRunnerSourceSet
        }
        guard let first = legs.first else { throw SP4AValidationError.missingLeg }
        for leg in legs {
            guard let rule = Phase0Registry.legRules[leg.legID], rule.evidenceKind == .fixture,
                  rule.evidenceKind == leg.evidenceKind, rule.detectorID == "D0", leg.detectorID == "D0" else {
                throw SP4AValidationError.invalidRule
            }
            guard leg.detectorAvailable, leg.blocker == nil, (leg.verdict == .pass || leg.verdict == .fail),
                  ...
                  let path = leg.artifactPath, SP4ADirectoryLayout.boundArtifactNames.contains(path),
                  !path.contains("/"), !path.contains(".."),
                  leg.artifactSha256?.isLowercaseSHA256 == true,
                  leg.artifactSha256 != String(repeating: "0", count: 64) else {
                throw SP4AValidationError.invalidVerdict
            }
        }
        let aggregate = legs.map(\.verdict).max { Self.precedence($0) < Self.precedence($1) } ?? .blocked
        guard verdict == aggregate else { throw SP4AValidationError.invalidAggregate }
    }
...
public enum SP4ARunnerBinding {
    public static let sourcePaths: Set<String> = [
        ...
        "Spikes/Sources/Phase0Probe/SP4AProbe.swift",
        ...
        "Spikes/Sources/Phase0Support/SP4AArtifacts.swift",
        "Spikes/Sources/Phase0Support/SP4AEvidence.swift",
        "Spikes/Sources/Phase0Support/ViaDefinitionParser.swift",
        "Spikes/Sources/Phase0Support/ViaDefinitionSources.swift",
        ...
        "Spikes/Sources/EvidenceValidator/SP4ADirectoryValidator.swift",
        "Spikes/Sources/EvidenceValidator/ValidatorError.swift",
    ]
}

public enum SP4ADirectoryLayout {
    public static let artifactNames: Set<String> = [
        "SP-4A-CONCLUSION.md", "bounds.json", "evidence.json", "opaque-preservation.json",
        "source-facts.json", "v2-schema.json", "v3-schema.json",
    ]
    public static let allNames = artifactNames.union(["manifest.sha256"])
    public static let boundArtifactNames = artifactNames.subtracting(["SP-4A-CONCLUSION.md", "evidence.json", "source-facts.json"])
    public static let legArtifacts = [
        "sp4a.v2Schema": "v2-schema.json",
        "sp4a.v3Schema": "v3-schema.json",
        "sp4a.opaqueRoundTrip": "opaque-preservation.json",
        "sp4a.bounds": "bounds.json",
    ]
}
